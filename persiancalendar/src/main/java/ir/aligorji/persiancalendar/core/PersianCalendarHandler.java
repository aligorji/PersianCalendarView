package ir.aligorji.persiancalendar.core;
...
import ir.aligorji.persiancalendar.R;
import ir.aligorji.persiancalendar.core.exceptions.DayOutOfRangeException;
...
import ir.aligorji.persiancalendar.core.interfaces.OnEventUpdateListener;
import ir.aligorji.persiancalendar.core.interfaces.OnMonthChangedListener;
import ir.aligorji.persiancalendar.core.models.AbstractDate;
import ir.aligorji.persiancalendar.core.models.CalendarEvent;
import ir.aligorji.persiancalendar.core.models.CivilDate;
import ir.aligorji.persiancalendar.core.models.Day;
import ir.aligorji.persiancalendar.core.models.IslamicDate;
import ir.aligorji.persiancalendar.core.models.PersianDate;
import ir.aligorji.persiancalendar.helpers.ArabicShaping;
import ir.aligorji.persiancalendar.helpers.DateConverter;
...
public class PersianCalendarHandler
{
    ...
    @DrawableRes
    private int mEventBackground = R.drawable.circle_select;
    @DrawableRes
    ...
    private boolean mIncludeOfficialEvents = false;
    private boolean mIsSelectableItem = false;
    ...
    private List<CalendarEvent> readEventsFromJSON()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                result.add(new CalendarEvent(new PersianDate(year, month, day), title, holiday, 0));
            }
    ...
    public List<CalendarEvent> getAllEventsForDay(PersianDate day)
    {
        ...
        if (includeOfficialEvents())
        {
            events = getOfficialEventsForDay(day);
        }
        else
        {
    ...
    public List<Day> getDays(int offset)
    {
        ...
        try
        {
            ...
            for (int i = 1; i <= 31; i++)
            {
                ...
                {
                    final List<CalendarEvent> localEventsForDay = getLocalEventsForDay(persianDate);
                    if (localEventsForDay.size() > 0)
                    {
                        day.setEvent(true, true, localEventsForDay.get(0).getEventBackground());
                    }
                }
                if (mHighlightOfficialEvents && includeOfficialEvents())
                {
                    final List<CalendarEvent> officialEventsForDay = getOfficialEventsForDay(persianDate);
                    if (officialEventsForDay.size() > 0)
                    {
                        day.setEvent(true, false, officialEventsForDay.get(0).getEventBackground());
                    }
                }
    ...
    public PersianCalendarHandler setEventBackground(int selectedDayBackground)
    {
        mEventBackground = selectedDayBackground;
        return this;
    }

    public void setSelectableItem(boolean mIsSelectableItem)
    {
        this.mIsSelectableItem = mIsSelectableItem;
    }

    public boolean isSelectableItem()
    {
        return this.mIsSelectableItem;
    }
    public void setIncludeOfficialEvents(boolean v)
    {
        this.mIncludeOfficialEvents = v;
    }

    public boolean includeOfficialEvents()
    {
        return this.mIncludeOfficialEvents;
    }
}
