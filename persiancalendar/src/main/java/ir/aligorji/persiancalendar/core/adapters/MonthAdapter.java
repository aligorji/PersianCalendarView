package ir.aligorji.persiancalendar.core.adapters;
...
import ir.aligorji.persiancalendar.core.Constants;
import ir.aligorji.persiancalendar.core.PersianCalendarHandler;
import ir.aligorji.persiancalendar.core.fragments.MonthFragment;
...
public class MonthAdapter extends RecyclerView.Adapter<MonthAdapter.ViewHolder>
{
    ...
    public void onBindViewHolder(MonthAdapter.ViewHolder holder, int position)
    {
        ...
        holder.mEvent.setBackgroundResource(mCalendarHandler.getEventBackground());
        position += 6 - (position % 7) * 2;
        if (mTotalDays < position - 6 - mFirstDayOfWeek)
        {
            return;
        ...
        if (!isPositionHeader(position))
        {
            if (position - 7 - mFirstDayOfWeek >= 0)
            {
                ...
                if (day.isEvent())
                {
                    ...
                    {
                        if (day.getEventBackground() > 0)
                        {
                            holder.mEvent.setBackgroundResource(day.getEventBackground());
                        }
                ...
                if (mCalendarHandler.isSelectableItem())
                {
